...
Objective:
Method 1 re.sub()
- Focusing on understanding how the re.sub() method from Python's regular expression module can be applied for string manipulation, such as cleaning text by removing unwanted characters.
- The function cleanup_text() uses the re lib to clean a string by:
- removing non-letter characters except full stops and spaces.
- Converting all text to uppercase, and replacing multispaces with a single space
- cleaned_txt = re.sub(r'[^A-Za-z. ]', '', text).upper() removes non-character letters
-  cleaned_txt = re.sub(r'\s+', ' ', cleaned_txt) removes multiple spaces and converts them into one.
- re.sub() Method:
My understanding of reading the following link on Python's documentation:
...
-     prefix = result[-2:] 
PURPOSE:
- Extracts the last two characters of the string to use as a prefix for checking for matching trigrams 
...
- counts counts how often each of the possible next chars follow the prefix from the fill.
